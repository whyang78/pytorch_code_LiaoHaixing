...
#wgan最重要的地方 限制判决器
def gradient_penalty(D, xr, xf, LAMBDA=0.3):
    '''
    :param D:判决器
    :param xr: 判决器真实输入数据
    :param xf: 判决器假的输入数据(生成器生成的数据)
    :return:
    '''
    xf = xf.detach()
    xr = xr.detach()

    alpha = torch.rand((xf.size(0), 1,1,1)).cuda()
...
for e in range(epoch):
    for i, (datas, _) in enumerate(trainDataLoader):
        ...
        loss_d=d_loss(logits_real,logits_fake)+gradient_penalty(d_net,datas,fake)
